...
while True:
    response = supabase.table("products").select("id,slug,manufacturer,name").range(offset, offset + limit - 1).execute()
...
for i, product in enumerate(products_with_cyrillic, 1):
    ...
    if new_slug != old_slug:
        ...
        if len(batch) >= BATCH_SIZE:
            for item in batch:
                try:
                    supabase.table("products").update({"slug": item['new_slug']}).eq("id", item['id']).execute()
                    fixed_count += 1

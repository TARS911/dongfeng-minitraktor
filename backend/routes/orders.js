...
export default async function orderRoutes(fastify, options) {
  ...
  fastify.post('/orders', {
    schema: {
      body: {
        type: 'object',
        required: ['customer_name', 'customer_phone', 'delivery_city', 'items'],
        properties: {
    ...
    const transaction = db.transaction((orderData) => {
      try {
        const { customer_name, customer_phone, customer_email, delivery_address, delivery_city, notes, items } = orderData;
        ...
        for (const item of items) {
          const product = db.prepare('SELECT id, name, model, price, in_stock FROM products WHERE id = ?').get(item.product_id);

...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    ...
    console.error('');
    console.error('Создайте файл .env в папке backend/database/ со следующим содержимым:');
    console.error('');
...
export const config = {
    ...
    sqliteDbPath: process.env.SQLITE_DB_PATH || path.join(__dirname, 'dongfeng.db')
};
...
export const validators = {
    validateCategory(category) {
        if (!category.name || typeof category.name !== 'string') {
            throw new Error(`Некорректное имя категории: ${JSON.stringify(category)}`);
        }
        if (!category.slug || typeof category.slug !== 'string') {
            throw new Error(`Некорректный slug категории: ${JSON.stringify(category)}`);
        }
        return true;
    },
    ...
    validateProduct(product) {
        ...
        if (!product.price || product.price <= 0) {
            throw new Error(`Некорректная цена для ${product.name}: ${product.price}`);
        }
        if (!product.power || product.power <= 0) {
            throw new Error(`Некорректная мощность для ${product.name}: ${product.power}`);
        }
        return true;
    },
    ...
    validateContact(contact) {
        ...
        if (!contact.phone || typeof contact.phone !== 'string') {
            throw new Error(`Некорректный телефон контакта: ${JSON.stringify(contact)}`);
        }
        return true;
    },
...
// Утилита для создания резервной копии SQLite

...
async function submitCallbackForm(event) {
  ...
  try {
    ...
    if (data.success) {
      ...
      alert(
        "Ошибка при отправке заявки. Попробуйте еще раз или позвоните нам: +7 (969) 999-56-68",
      );
    }
  } catch (error) {
...
async function submitServiceForm(event) {
  ...
  try {
    ...
    if (data.success) {
      alert(
        "Спасибо! Ваша заявка на сервисное обслуживание принята.\nМы свяжемся с вами в ближайшее время.",
      );
      form.reset();
...
async function submitWarrantyForm(event) {
  ...
  try {
    ...
    if (data.success) {
      ...
      alert(
        "Ошибка при отправке заявки. Попробуйте еще раз или позвоните на горячую линию: 8 (495) 069-7915",
      );
    }
  } catch (error) {
    console.error("Error:", error);
...
// === Catalog Menu Toggle (для кнопки каталога в header) ===
...
console.log("%cОфициальный дилер в России", "color: #666; font-size: 14px;");
